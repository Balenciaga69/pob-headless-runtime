...
testkit.expect(
    selectedSkill.skill and type(selectedSkill.skill.name) == "string",
    "importer_update: expected baseline selected skill"
)
...
testkit.expect(
    importTab.controls.accountName ~= nil,
    "importer_update: expected account name control"
)
...
importTab.DownloadPassiveTree = function(self)
    ...
    testkit.expect(
        passiveData ~= nil and passiveErr == nil,
        "importer_update: expected passive importer fixture to parse"
    )
    self.charImportMode = "SELECTCHAR"
end
...
testkit.expect(
    result.restoredSkillSelection == true,
    "importer_update: expected skill restore after remote import"
)
testkit.expect(
    result.skillSelection
        and result.skillSelection.skill
        and type(result.skillSelection.skill.name) == "string",
    "importer_update: expected restored skill selection"
)
testkit.expect(#observedCalls == 3, "importer_update: expected remote importer download sequence")
testkit.expect(
    observedCalls[1] == "characters",
    "importer_update: expected character list download first"
)
testkit.expect(
    observedCalls[2] == "passive",
    "importer_update: expected passive tree download second"
)
testkit.expect(observedCalls[3] == "items", "importer_update: expected items download third")
testkit.expect(
    updatedSummary.level == 100,
    "importer_update: expected imported character level to update"
)
testkit.expect(
    type(updatedSummary.mainSkill) == "string" and updatedSummary.mainSkill ~= "",
    "importer_update: expected summary main skill to remain restored"
)
testkit.expect(
    testkit.summaryStat(updatedSummary, "Life") ~= testkit.summaryStat(baselineSummary, "Life"),
    "importer_update: expected importer smoke to change life"
)
