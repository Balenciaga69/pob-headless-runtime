...
smokekit.runQueuedSmoke(api, xmlPath, function(flow, baselineSummary)
    ...
    testkit.expect(
        type(tree.nodes) == "table" and #tree.nodes > 0,
        "tree_simulation: expected get_tree nodes payload"
    )
    ...
    testkit.expect(
        snapshot.kind == "tree_snapshot",
        "tree_simulation: expected explicit tree snapshot"
    )
    ...
    testkit.expect(
        queriedNode.id == candidateNodeId,
        "tree_simulation: expected queried tree node id"
    )
    testkit.expect(
        queriedNode.allocated == false,
        "tree_simulation: expected candidate node to be unallocated before simulation"
    )

    local nodeName = queriedNode.displayName or queriedNode.name
    testkit.expect(
        type(nodeName) == "string" and nodeName ~= "",
        "tree_simulation: expected searchable node name"
    )
    ...
    testkit.expect(
        type(searchResult.nodes) == "table" and #searchResult.nodes >= 1,
        "tree_simulation: expected search results"
    )
    ...
    testkit.expect(
        (allocatedSearch.total or 0) >= 1,
        "tree_simulation: expected allocated notable search results"
    )
    ...
    testkit.expect(
        missingNodeErr == "node not found",
        "tree_simulation: expected missing node error"
    )
    ...
    testkit.expect(
        simulated.restored == true,
        "tree_simulation: expected tree simulation restore flag"
    )
    testkit.expect(
        simulated.simulationMode == "snapshot_restore",
        "tree_simulation: expected tree simulation mode"
    )
    testkit.expect(type(simulated.tree) == "table", "tree_simulation: expected tree payload")
    testkit.expect(
        type(simulated.tree.before) == "table",
        "tree_simulation: expected before tree payload"
    )
    testkit.expect(
        type(simulated.tree.after) == "table",
        "tree_simulation: expected after tree payload"
    )
    testkit.expect(
        type(simulated.tree.target) == "table",
        "tree_simulation: expected target tree payload"
    )
    testkit.expect(
        type(simulated.tree.requestedChanges) == "table",
        "tree_simulation: expected requested tree changes payload"
    )
    testkit.expect(
        type(simulated.delta) == "table",
        "tree_simulation: expected top-level delta payload"
    )

    testkit.expect(
        type(simulated.delta.Life) == "number",
        "tree_simulation: expected numeric life delta"
    )
    testkit.expect(
        type(simulated.delta.EnergyShield) == "number",
        "tree_simulation: expected numeric ES delta"
    )
    testkit.expect(
        type(simulated.delta.CombinedDPS) == "number",
        "tree_simulation: expected numeric DPS delta"
    )
    ...
    testkit.expect(
        testkit.normalizeNumber(baselineSummary.stats.Life)
            == testkit.normalizeNumber(afterSummary.stats.Life),
        "tree_simulation: expected tree simulation to restore life"
    )
    testkit.expect(
        testkit.normalizeNumber(baselineSummary.stats.CombinedDPS)
            == testkit.normalizeNumber(afterSummary.stats.CombinedDPS),
    ...
    testkit.expect(
        #(restored.tree.nodes or {}) == #(tree.nodes or {}),
        "tree_simulation: expected explicit restore to preserve node count"
    )
