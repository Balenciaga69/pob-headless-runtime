...
smokekit.runQueuedSmoke(api, xmlPath, function(_, summary)
    ...
    testkit.expect(
        invalidTestSlotErr == "invalid item slot"
            or invalidTestSlotErr == "item cannot be equipped in slot Fake Slot",
        "item_slot_rules: expected invalid slot error"
    )
    ...
    testkit.expect(
        testedShield.slot and testedShield.slot.resolved == "Weapon 2",
        "item_slot_rules: expected shield to be valid in Weapon 2"
    )
    ...
    testkit.expect(
        testedOffhandWand.slot and testedOffhandWand.slot.resolved == "Weapon 2",
        "item_slot_rules: expected wand to be valid in Weapon 2"
    )

    local _, invalidSceptreErr = api.test_item(sceptreItemText, "Weapon 2")
    testkit.expect(
        invalidSceptreErr == "item cannot be equipped in slot Weapon 2",
        "item_slot_rules: expected sceptre to be invalid in Weapon 2"
    )

    local _, invalidBowOffhandErr = api.test_item(bowItemText, "Weapon 2")
    testkit.expect(
        invalidBowOffhandErr == "item cannot be equipped in slot Weapon 2",
        "item_slot_rules: expected bow to be invalid in Weapon 2"
    )

    local _, invalidQuiverErr = api.test_item(quiverItemText, "Weapon 2")
    testkit.expect(
        invalidQuiverErr == "item cannot be equipped in slot Weapon 2",
        "item_slot_rules: expected quiver to be invalid without bow main hand"
    )
    ...
    testkit.expect(
        testedBowMainhand.slot and testedBowMainhand.slot.resolved == "Weapon 1",
        "item_slot_rules: expected bow to be valid in Weapon 1"
    )
    ...
    testkit.expect(
        equippedBelt.slot and equippedBelt.slot.resolved == "Belt",
        "item_slot_rules: expected stygian belt equip slot"
    )
    testkit.expect(
        summary and summary.buildName ~= nil,
        "item_slot_rules: expected build summary before abyss socket check"
    )
    ...
    local testedAbyssJewel, testedAbyssJewelErr =
        api.test_item(abyssJewelItemText, "Belt Abyssal Socket 1")
    if not testedAbyssJewel then
        return false, testedAbyssJewelErr
    end
    testkit.expect(
        testedAbyssJewel.slot and testedAbyssJewel.slot.resolved == "Belt Abyssal Socket 1",
        "item_slot_rules: expected abyss jewel test slot"
    )

    local equippedAbyssJewel, equipAbyssJewelErr =
        api.equip_item(abyssJewelItemText, "Belt Abyssal Socket 1")
    if not equippedAbyssJewel then
        return false, equipAbyssJewelErr
    end
    testkit.expect(
        equippedAbyssJewel.slot and equippedAbyssJewel.slot.resolved == "Belt Abyssal Socket 1",
        "item_slot_rules: expected abyss jewel equip slot"
    )
    ...
    testkit.expect(
        equippedBow.slot and equippedBow.slot.resolved == "Weapon 1",
        "item_slot_rules: expected bow equip slot"
    )
    ...
    testkit.expect(
        testedQuiver.slot and testedQuiver.slot.resolved == "Weapon 2",
        "item_slot_rules: expected quiver to become valid with bow main hand"
    )
    ...
    testkit.expect(
        equippedQuiver.slot and equippedQuiver.slot.resolved == "Weapon 2",
        "item_slot_rules: expected quiver equip slot"
    )
    ...
    testkit.expect(
        beltSummary.buildName ~= nil,
        "item_slot_rules: expected summary after slot mutations"
    )
