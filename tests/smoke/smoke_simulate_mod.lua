...
smokekit.runQueuedSmoke(api, xmlPath, function(_, baselineSummary)
    local simulated, simulateErr =
        api.simulate_mod("+20 to maximum Life", "Ring 1", { "Life", "FireResist", "ColdResist" })
    if not simulated then
        return false, simulateErr
    end

    testkit.expect(
        simulated.modLine == "+20 to maximum Life",
        "simulate_mod: expected mod line echo"
    )
    testkit.expect(
        simulated.slot and simulated.slot.resolved == "Ring 1",
        "simulate_mod: expected simulate_mod slot"
    )
    ...
    testkit.expect(
        simulated.simulationMode == "calculator",
        "simulate_mod: expected mod simulation mode"
    )
    testkit.expect(
        type(simulated.comparison) == "table",
        "simulate_mod: expected comparison payload"
    )
    testkit.expect(
        type(simulated.comparison.delta) == "table",
        "simulate_mod: expected comparison delta"
    )
    ...
    testkit.expect(
        testkit.normalizeNumber(simulated.delta.Life) ~= 0,
        "simulate_mod: expected non-zero simulated life delta"
    )
