...
smokekit.runQueuedSmoke(api, xmlPath, function(_, baselineSummary)
    ...
    testkit.expect(
        compared.restored == true,
        "config_compare: expected config simulation restore flag"
    )
    testkit.expect(
        compared.simulationMode == "snapshot_restore",
        "config_compare: expected config simulation mode"
    )
    testkit.expect(
        type(compared.comparison) == "table",
        "config_compare: expected comparison payload"
    )
    testkit.expect(
        type(compared.comparison.delta) == "table",
        "config_compare: expected comparison delta"
    )
    testkit.expect(
        type(compared.comparison.fields) == "table" and #compared.comparison.fields == 3,
        "config_compare: expected compare fields"
    )
    testkit.expect(
        compared.comparison._meta and compared.comparison._meta.before,
        "config_compare: expected compare meta"
    )
    testkit.expect(
        type(compared.delta) == "table",
        "config_compare: expected top-level delta table"
    )
    testkit.expect(
        type(compared.before) == "table",
        "config_compare: expected top-level before table"
    )
    testkit.expect(
        type(compared.after) == "table",
        "config_compare: expected top-level after table"
    )
    testkit.expect(
        type(compared.changedFields) == "table",
        "config_compare: expected top-level changed fields table"
    )
    ...
    testkit.expect(
        ...
        "config_compare: expected invalid fields error"
    )
