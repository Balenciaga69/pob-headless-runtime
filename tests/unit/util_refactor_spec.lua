...
do
    ...
    local build, err =
        buildGuard.getBuildWithTabs(session, { "itemsTab", "calcsTab" }, "items not initialized")
    expect(
        build ~= nil and err == nil,
        "expected getBuildWithTabs to return build when all tabs exist"
    )
...
do
    ...
    expect(
        err == "items not initialized",
        "expected getBuildWithTabs to return caller error message"
    )
end
...
do
    ...
    local result, err = buildGuard.withBuild(
        session,
        { "configTab" },
        "build/config not initialized",
        function(build)
...
do
    ...
    expect(
        delta.ChaosResist == 15,
        "expected numericDelta to treat missing numeric before values as zero"
    )
    ...
    expect(
        changedFields[2] == "ChaosResist",
        "expected changedFields to include newly-added numeric field"
    )
end
