...
expect(contains(capabilityMatrix.deterministicFake, "Copy"), "expected Copy to be marked deterministic")
expect(contains(capabilityMatrix.unsupported, "OpenURL"), "expected OpenURL to be marked unsupported")
expect(contains(capabilityMatrix.compatibilityShim, "Inflate"), "expected Inflate to be marked compatibility shim")
...
expect(type(openErr) == "string" and openErr:match("OpenURL%(%).*unavailable"), "expected explicit OpenURL error")
...
expect(contains(exportedMatrix.safeNoOp, "RenderInit"), "expected exported matrix to include safeNoOp capabilities")
