...
do
	...
	expect(type(session.api.experimental.load_build_file) == "function", "expected experimental build helper namespace")
	expect(session.api.load_build_file == nil, "expected experimental helper to stay off stable root")
...
do
	...
	expect(type(err) == "string" and err:match("did not settle"), "expected bounded loop timeout error")
...
do
	...
	expect(err == "runUntilSettled() cannot be called from inside callbacks", "expected callback guard error")
end
...
do
		...
		callbacks.mainObject.main.modes[mode] = {
			importTab = {
				...
				ImportPassiveTreeAndJewels = function(_, treeJson, charData)
					...
					expect(charData and charData.name == "Offline", "expected imported character data to be reused")
				end,
			},
		}
	end
	...
	expect(type(PoBHeadless.load_build_file) == "function", "expected legacy flattened experimental helper")
	expect(type(PoBHeadless.experimental.load_build_file) == "function", "expected legacy namespaced experimental helper")
