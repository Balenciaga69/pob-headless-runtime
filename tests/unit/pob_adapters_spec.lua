...
do
    ...
    expect(
        importAdapter:get_import_tab({ importTab = importTab }) == importTab,
        "expected import tab lookup"
    )
    ...
    expect(
        importAdapter:get_selected_character(importTab).name == "Character",
        "expected selected character"
    )
    expect(
        charsOk == true and passiveOk == true and itemsOk == true,
        "expected adapter downloads to succeed"
    )
