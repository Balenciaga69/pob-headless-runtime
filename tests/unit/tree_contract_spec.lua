...
local function newTreeRepo()
	...
	build.spec.ImportFromNodeList = function(_, classId, ascendClassId, secondaryAscendClassId, nodes, _, masteryEffects, treeVersion)
...
do
	local treeRepo, state = newTreeRepo()
...
do
	...
	expect(restored.tree.nodes[1] == 100 and restored.tree.nodes[2] == 200, "expected restore node list")
end
...
do
	...
	expect(result.tree.before.nodes[1] == 100 and result.tree.before.nodes[2] == 200, "expected before tree nodes")
	expect(result.tree.after.nodes[1] == 100 and result.tree.after.nodes[2] == 300 and result.tree.after.nodes[3] == 400, "expected after tree nodes")
	...
	expect(result.tree.addedNodes[1] == 300 and result.tree.addedNodes[2] == 400, "expected added node list")
