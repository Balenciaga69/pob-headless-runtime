...
do
    ...
    expect(
        result.skillSelection == snapshot,
        "expected returned skill selection to match restored snapshot"
    )
    expect(
        calls.lastBuild and calls.lastBuild.source == "offline",
        "expected offline build payload"
    )
end
