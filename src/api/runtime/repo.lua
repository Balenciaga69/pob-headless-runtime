...
function M:rebuild_config(build)
    ...
    build = build
        or self:ensure_build_ready({ "configTab", "calcsTab" }, "build/config not initialized")
    if not build then
...
function M:get_main_object_main()
    ...
    return accessUtil.getMainObjectMain(
        self.session.callbacks and self.session.callbacks.mainObject or nil
    )
end
