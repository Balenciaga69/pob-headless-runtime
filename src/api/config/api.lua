...
local function getConfigService(session)
    -- Look up the config orchestrator from the bound session.
...
local function requireConfigService(session)
    -- Fail fast when the config orchestrator is unavailable.
...
function M.set_config(session, params)
    -- Apply a config patch through the orchestrator layer.
