...
local function normalizeNodeDeltaParams(params)
    ...
    return {
        ...
        secondaryAscendClassId = params.secondaryAscendClassId ~= nil and tonumber(
            params.secondaryAscendClassId
        ) or nil,
...
local function diffMasteryEffects(beforeEffects, afterEffects)
    ...
    for masteryId, effectId in pairs(afterById) do
        if beforeById[masteryId] ~= effectId then
            changed[#changed + 1] =
                { masteryId = masteryId, before = beforeById[masteryId], after = effectId }
        end
    end
...
local function buildMeta(statsRepo, build)
    ...
    return {
        ...
        treeVersion = build
            ...
            or nil,
        mainSkill = statsRepo and statsRepo.get_main_skill_name and statsRepo:get_main_skill_name(
            build
        ) or nil,
    }
end
...
function M:summarize_node(spec, nodeId, node)
    ...
    return {
        ...
        stats = stats,
        masteryEffects = masteryEffects,
        selectedMasteryEffectId = spec and spec.masterySelections and tonumber(
            spec.masterySelections[nodeId]
        ) or nil,
    }
end
...
function M:restore_snapshot(snapshot)
    ...
    local build, err =
        self.runtime:ensure_build_ready({ "spec", "calcsTab" }, "build/spec not initialized")
    if not build then
        return nil, err
    end
...
function M:build_target_tree(currentTree, params)
    ...
    return {
        ...
        secondaryAscendClassId = params.secondaryAscendClassId
            or currentTree.secondaryAscendClassId,
        nodes = nodes,
        masteryEffects = masteryEffects,
    }
end
...
function M:simulate_delta(params, fields)
    ...
    if
        not build.spec.CreateUndoState
        or not build.spec.RestoreUndoState
        or not build.spec.ImportFromNodeList
    then
    ...
    local beforeStats = pickFields(
        beforeOutput,
        fields,
        self.stats and self.stats.get_default_stat_fields and self.stats:get_default_stat_fields()
            or nil
    )
    ...
    local changedMasteryEffects =
        diffMasteryEffects(beforeTree.masteryEffects, afterTree.masteryEffects)
    ...
    return simulationUtil.buildResult("tree", compared, {
        ...
        tree = {
            ...
            target = {
                ...
                masteryEffects = normalizeMasteryEffects(targetTree.masteryEffects)
                        and summarizeMasteryEffects({
                            masterySelections = targetTree.masteryEffects,
                        })
                    or {},
            },
            ...
            requestedChanges = {
                ...
                masteryEffects = normalizedParams.masteryEffects
                        and summarizeMasteryEffects({
                            masterySelections = normalizedParams.masteryEffects,
                        })
                    or {},
            },
        },
