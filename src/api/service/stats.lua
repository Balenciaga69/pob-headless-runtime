-- Stats service that normalizes field selection and reads.
...
function M:get_default_stat_fields()
	return self.repos.stats:get_default_stat_fields()
end
...
function M:build_meta(build)
	return {
		...
		mainSkill = self.repos.stats:get_main_skill_name(build),
	}
end
...
function M:get_stats(fields)
	local output, build = self.repos.stats:get_output()
	if not output then
		return nil, build
	end
...
function M:normalize_compare_fields(beforeStats, afterStats, fields)
	...
	if #normalized == 0 then
		for key, _ in pairs(beforeStats or {}) do
			if type(key) == "string" and key ~= "_meta" and type((beforeStats or {})[key]) == "number" then
				normalized[#normalized + 1] = key
				seen[key] = true
			end
		end
		for key, _ in pairs(afterStats or {}) do
			if type(key) == "string" and key ~= "_meta" and type((afterStats or {})[key]) == "number" and not seen[key] then
				normalized[#normalized + 1] = key
				seen[key] = true
			end
		end
