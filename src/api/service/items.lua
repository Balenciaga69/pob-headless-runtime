...
local DEFAULT_ITEM_COMPARE_FIELDS = {"TotalDPS", "CombinedDPS", "Life", "EnergyShield", "Mana", "Armour", "Evasion",
...
function M:test_item(itemText, slot)
	...
	return {
		...
		delta = statsCompareUtil.numericDelta(simulation.baseOutput, simulation.newOutput, DEFAULT_ITEM_COMPARE_FIELDS),
	}
end
...
function M:compare_item_stats(itemText, slot, fields)
	...
	local compared, compareErr = self.services.stats:compare_stats(beforeStats, afterStats, compareFields)
...
function M:simulate_mod(modLine, slot, fields)
	...
	local candidateRaw, equippedOrErr = self.repos.items:build_candidate_raw_with_appended_mod(slot, modLine)
