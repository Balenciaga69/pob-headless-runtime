...
function M:simulate_node_delta(params, fields)
    ...
    tree.removedNodes = tree.removedNodes
        or select(2, diffNodeLists(beforeTree.nodes, afterTree.nodes))
    tree.changedMasteryEffects = tree.changedMasteryEffects
        or diffMasteryEffects(beforeTree.masteryEffects, afterTree.masteryEffects)
    result.tree = tree
    return result
end
