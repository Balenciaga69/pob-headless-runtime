-- Tree orchestrator that packages snapshot, compare, and delta flows.
...
function M:simulate_node_delta(params, fields)
    ...
    if not afterOutput then
        self:restore_tree_snapshot({ state = snapshot })
        return nil, applyErr
    end
    ...
    local _, restoreErr = self:restore_tree_snapshot({ state = snapshot })
    if restoreErr then
