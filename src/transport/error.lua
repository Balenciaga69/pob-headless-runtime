...
local function parseTimeoutDetails(text)
	local maxFrames, maxSeconds, pending, mainReady, buildReady, calcsReady, outputReady = text:match(
...
function M.fromUpstream(id, message, meta)
	...
	if text:match("unsupported config field") then
		return M.response(id, M.codes.UNSUPPORTED_FIELD, text, nil, parseUnsupportedFieldDetails(text), meta)
	end
	if text:match("build not initialized")
		or text:match("build/config not initialized")
		or text:match("items not initialized")
	then
		return M.response(id, M.codes.BUILD_NOT_READY, text, nil, parseBuildNotReadyDetails(text), meta)
	end
